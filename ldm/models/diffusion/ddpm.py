...
class DDPM(pl.LightningModule): #xsm: modified for masking
    ...
    def from_args(cls, unet_config, args):
        return cls(unet_config=unet_config,
    ...
    def get_loss_masked(self, pred_img, pred_sunny, pred_inval, target_img, target_sunny, target_inval, 
        ...
        if self.loss_type == 'l1':
            ...
            if mean:
                ...
                torch.nn.functional.mse_loss(target_sunny, pred_sunny) + alpha_inval * \
                torch.nn.functional.mse_loss(target_inval, pred_inval)
            else:
                loss = torch.nn.functional.mse_loss(target_img_masked, pred_img_masked, reduction='none') / mask_ratio + alpha_sunny * \
                torch.nn.functional.mse_loss(target_sunny, pred_sunny, reduction='none') + alpha_inval * \
                torch.nn.functional.mse_loss(target_inval, pred_inval, reduction='none')
        else:
            raise NotImplementedError("unknown loss type '{loss_type}'")
        return loss
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        loss_dict = {}
        if self.parameterization == "eps": #xsm: Predict noise or initial image?
            target = noise
        elif self.parameterization == "x0":
            target = x_start
        else:
    ...
    def p_losses_masked(self, x_start, t, noise=None, cond=None, mask=None, alpha_sunny=0.1, alpha_inval=0.1):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        img_out, sunny_out, inval_out = self.model(x_noisy, t, cond, mask['invalid_mask'])
        ...
        if self.parameterization == "eps":
            ...
            target_img = noise
        target_sunny = mask['sunny_mask']
        target_inval = mask['ori_invalid_mask']
        
        loss = self.get_loss_masked(img_out, sunny_out, inval_out, target_img, target_sunny, target_inval, alpha_sunny, alpha_inval, mean=False).mean(dim=[1, 2, 3])
...
class LatentDiffusion(DDPM): #xsm: main latent!
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
